function fish_prompt --description 'Write out the prompt'
 ...
 (git status --porcelain | cut -c 1-2 | uniq)
    ...
    set_color -b black
    printf '%s%s%s%s%s%s%s%s%s%s%s%s%s' (set_color -o white) '❰' (set_color green) $USER (set_color white) '❙' (set_color yellow) (echo $PWD | sed -e "s|^$HOME|~|" | sed -e "s|~/.ghq/github.com|ghq|") (set_color white) $git_info (set_color white) '❱' (set_color white)
